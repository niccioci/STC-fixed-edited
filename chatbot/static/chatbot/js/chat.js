...
$(document).ready(function () {
  ...
  function appendBotMessage(data, periodicAdvice) {
    ...
    if (
      ...
      $("#result_div_notification").append(
        "<p id='bot-message'>" + message + "</p><br>",
      );
      $("#result_div_notification #typing-gif").remove();
    ...
    setTimeout(function () {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          $("#available-balance-amount").html(
            response.available_balance_amount,
          );
  ...
  function addSuggestions(suggestions) {
    setTimeout(function () {
      ...
      if (conditionActive) {
        ...
        $('<div class="row suggestion-row-notification"></div>').appendTo(
          "#result_div_notification",
        );
        ...
        for (i = 0; i < suggestions.length; i++) {
          $(
            '<p class="sugg-options">' + suggestions[i].title + "</p>",
          ).appendTo(".suggestion-row");
          if (addToNotification) {
            console.log("adding notifications");
            $(
              '<p class="sugg-options-notification">' +
                suggestions[i].title +
                "</p>",
            ).appendTo(".suggestion-row-notification");
          }
        }
        ...
        if (addToNotification) {
          suggestionRowNotificationHeight = $(
            ".suggestion-row-notification",
          ).height();
          resultDivNotificationHeight =
            $(".notification").height() -
            (90 + suggestionRowNotificationHeight);
          $("#result_div_notification").css(
            "height",
            resultDivNotificationHeight,
          );
          $("#result_div_notification").scrollTop(
    ...
    if (!secondPeriodicAdvice) {
      ...
      if (message) {
        ...
        var post_data = {
          message: message,
          sender: username,
          periodic_advice: periodicAdvice,
          month: month,
        ...
        if (!periodicAdvice) {
          $("#result_div").append(
            "<p id='user-message'> " + message + "</p><br>",
          );
          $("#result_div").scrollTop($("#result_div")[0].scrollHeight);
          $("#chat-input").val("");
          ...
          if ($("#result_div_notification").is(":visible")) {
            $("#result_div_notification").append(
              "<p id='user-message-notification'> " + message + "</p><br>",
            );
            $("#result_div_notification").scrollTop(
              $("#result_div_notification")[0].scrollHeight,
            );
